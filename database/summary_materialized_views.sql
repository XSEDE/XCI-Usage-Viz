...
CREATE INDEX ON sum_usage_user_monthly USING BTREE (used_component);
CREATE INDEX ON sum_usage_user_monthly USING BTREE (use_month);
CREATE INDEX ON sum_usage_user_monthly USING BTREE (use_user);
...
CREATE INDEX ON sum_usage_user_daily USING BTREE (used_component);
CREATE INDEX ON sum_usage_user_daily USING BTREE (use_day);
CREATE INDEX ON sum_usage_user_daily USING BTREE (use_user);
...
CREATE INDEX ON sum_usage_user_hourly USING BTREE (used_component);
CREATE INDEX ON sum_usage_user_hourly USING BTREE (use_hour);
CREATE INDEX ON sum_usage_user_hourly USING BTREE (use_user);
...
CREATE INDEX ON sum_usage_user_monthly USING BTREE (used_component);
CREATE INDEX ON sum_usage_monthly USING BTREE (use_month);
...
CREATE INDEX ON sum_usage_daily USING BTREE (used_component);
CREATE INDEX ON sum_usage_daily USING BTREE (use_day);
...
CREATE MATERIALIZED VIEW sum_usage_hourly (used_component, use_hour, use_count, use_distinct_users, use_amount, use_amount_units)
    AS SELECT used_component, use_hour, sum(use_count), count(distinct(use_user)), sum(use_amount), use_amount_units
        FROM sum_usage_user_hourly
        GROUP BY used_component, use_hour, use_amount_units
        ORDER BY used_component, use_hour;

CREATE INDEX ON sum_usage_hourly USING BTREE (used_component);
CREATE INDEX ON sum_usage_hourly USING BTREE (use_hour);
...
CREATE MATERIALIZED VIEW sum_usage_user_client_monthly (used_component, use_month, use_user, use_client, use_count)
    AS SELECT used_component, DATE_TRUNC('month', use_timestamp) AS use_month, use_user, use_client, count(*)
        FROM std_usage_entry
        WHERE use_timestamp < DATE_TRUNC('month', now())
          AND used_component = 'org.cilogon.auth'
        GROUP BY used_component, use_month, use_user, use_client
        ORDER by used_component, use_month;

CREATE INDEX ON sum_usage_user_client_monthly USING BTREE (used_component);
CREATE INDEX ON sum_usage_user_client_monthly USING BTREE (use_month);
CREATE INDEX ON sum_usage_user_client_monthly USING BTREE (use_user);
...
CREATE INDEX ON sum_usage_user_client_daily USING BTREE (used_component);
CREATE INDEX ON sum_usage_user_client_daily USING BTREE (use_day);
CREATE INDEX ON sum_usage_user_client_daily USING BTREE (use_user);
...
CREATE INDEX ON sum_usage_user_client_hourly USING BTREE (used_component);
CREATE INDEX ON sum_usage_user_client_hourly USING BTREE (use_hour);
CREATE INDEX ON sum_usage_user_client_hourly USING BTREE (use_user);
...
CREATE INDEX ON sum_usage2_monthly USING BTREE (used_component);
CREATE INDEX ON sum_usage2_monthly USING BTREE (use_month);
...
CREATE INDEX ON sum_usage2_daily USING BTREE (used_component);
CREATE INDEX ON sum_usage2_daily USING BTREE (use_day);
...
CREATE INDEX ON sum_usage2_hourly USING BTREE (used_component);
CREATE INDEX ON sum_usage2_hourly USING BTREE (use_hour);
